...
def print_modified(dir):
    ...
    assert path.is_dir()
    bands = os.listdir(dir)     #  https://stackoverflow.com/questions/7099290/how-to-ignore-hidden-files-using-os-listdir
    for band in bands:
        ...
        if not band.startswith("."):         # discards hidden directories
            ...
            if (now - last_modified) > 0 and (now - last_modified) < 180:   #changes in last 3 minutes
